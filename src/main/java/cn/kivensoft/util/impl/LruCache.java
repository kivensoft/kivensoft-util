...
import cn.kivensoft.util.Cache;
...
final public class LruCache<K, V> implements Cache<K, V> {
	...
	public boolean containsKey(K key) {
		if (lock == null) return data.containsKey(key);
		else synchronized (lock) { return data.containsKey(key); }
	}
	...
	private static class CacheObject<K, V> {
		final K key;
		V value;
		long expire;
